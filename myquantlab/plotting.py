...
Docs: https://vtecftwy.github.io/myquantlab/plotting.html.md"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs-dev/03_plotting.ipynb.

# %% auto #0
__all__ = ['plot_timeseries', 'plot_two_color_line', 'plot_multi_color_line', 'hist_timeseries', 'plot_acfs',
...
# %% ../nbs-dev/03_plotting.ipynb #1dfb7b93
def plot_multi_color_line(
    ax: plt.Axes,       # Axes where to plot
    s: pd.Series,       # Pandas Series to plot
    cond_list: list     # List of boolean mask (True or False for each rowm in s)
    ):
    """"""
    import matplotlib.dates as mdates
    from matplotlib.collections import LineCollection
    colors = plt.cm.tab10(range(len(cond_list)))
    state_idx = np.select(cond_list, list(range(len(cond_list))), default=-1)
    seg_colors = colors[state_idx]
    x = mdates.date2num(np.asarray(s.index))
    y = np.asarray(s)
    pts = np.column_stack([x, y]).reshape(-1, 1, 2)
    segs = np.concatenate([pts[:-1], pts[1:]], axis=1)

    lc = LineCollection(segs, colors=seg_colors[:-1], linewidths=1)
    ax.add_collection(lc)
